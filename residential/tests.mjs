...
assert.equal(Math.round(result.netProfit), 13693);
...
const taxed = calculateDeal({ ...DEFAULTS, purchasePrice: 720000, taxAllowancePercent: 28 });
assert.equal(Math.round(taxed.taxAllowance), Math.round(taxed.netProfitBeforeTax * 0.28));
assert.equal(Math.round(taxed.netProfit), Math.round(taxed.netProfitBeforeTax * 0.72));
